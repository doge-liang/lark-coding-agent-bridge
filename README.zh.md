...
后台服务会先启动当前 profile 的升级 launcher：`~/.lark-channel/profiles/<profile>/upgrades/launcher.mjs`。没有升级状态时，launcher 会回退到注册服务时记录的 CLI 路径；完成自更新后，它会启动已选中的 release。
...
| `用量` | `/usage` |
| `Bunny` | 打开 Bunny 首页卡片 |
| `Bunny 选题` | 运行 Bunny 选题研究 |
| `Bunny 草稿` | 生成 Bunny 草稿 |
| `Bunny 审稿` | 打开 Bunny 审核队列 |
| `Bunny 排期` | 排期已批准的 Bunny 草稿 |
| `Bunny 日报` | 生成 Bunny 日报 |
| `状态` | `/status` |
| `新会话` | `/new` |
| `恢复` | `/resume` |
| `帮助` | `/help` |
...
底座。可在 Codex profile 里通过飞书悬浮菜单文案 `Bunny` 打开，然后使用
Bunny 首页卡片按钮，或 `Bunny 选题` 这类精确菜单文案触发业务动作。这个入口
只把飞书动作路由到独立的 Bunny Codex session；它不是旧的 Bunny 本地控制
server，也不是用户侧 CLI 命令组。
...
Codex profile 的 owner/admin 也可以在飞书里发送 `/codex-config`，或通过悬浮菜单项 `Codex 设置` 打开表单卡片，修改 `workspaces.default`、canonical permissions、Codex home 模式、`model`、`model_reasoning_effort`、`ignoreUserConfig` 和 `ignoreRules`。模型设置会写入当前 profile 选中的 Codex home 下的 `config.toml`。

Claude profile 的 owner/admin 则可以发送 `/claude-config`，或通过悬浮菜单项 `Claude 设置` 打开表单卡片，修改 `workspaces.default`、canonical permissions、`permissions.claude.permissionMode`、`claude.model` 和 `claude.approvalTimeoutMinutes`。设置从新的 Claude 会话开始生效。
...
| `~/.lark-channel/profiles/<profile>/upgrades/` | 自更新 launcher、状态、release、staging 和日志 |
