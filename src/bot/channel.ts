...
async function awaitRenderAwareStream(input: {
  ...
  if (!terminal) {
    ...
    // Stream updates are throttled inside @larksuite/channel: controller calls
    // return after updating local state, while the final PATCH happens when the
    // stream completes. If that PATCH stalls past the grace window, the visible
    // message can remain on the previous "running/tool" footer. Send a
    // terminal fallback so users see the finished state even if the original
    // stream transport is slow or stuck.
    await runFallbackReply(input.mode, first.state, input.fallback);
    return;
  }
